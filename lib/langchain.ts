...
export const indexName = "profpython";
...
export async function generateEmbeddingsInPineconeVectorStore(docId: string) {
  ...
  if (namespaceAlreadyExists) {
    ...
    pineconeVectorStore = await PineconeStore.fromExistingIndex(embeddings, {
      /* @ts-expect-error: Ignoring TypeScript error due to complex type inference for pineconeIndex */
      pineconeIndex: index,
      namespace: docId,
    });
    ...
    pineconeVectorStore = await PineconeStore.fromDocuments(
      ...
      embeddings,
      {
        /* @ts-expect-error: TypeScript cannot infer the type for pineconeIndex, but it is valid in this context */
        pineconeIndex: index,
        namespace: docId,
      }
    );
...
const generateLangchainCompletion = async (docId: string, question: string) => {
  ...
  const pineconeVectorStore = await (async () => {
    const store = await generateEmbeddingsInPineconeVectorStore(docId);
    if (!store) {
      throw new Error("Pinecone vector store not found");
    }
    return store;
